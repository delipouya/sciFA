...
varimax_df = read.csv('~/sciFA/Results/varimax_loading_df_stimPBMC.csv')

head(varimax_df)

df = data.frame(gene= varimax_df$X,factor=varimax_df$F3)
...
ggplot(enrich_res_pos, aes(y=term_name,x=log_p))+geom_bar(stat = 'identity')+xlab('-log(p value)')+
  theme_classic()+ylab('')+ggtitle(paste0(title, '(positive loading)'))
...
ggplot(enrich_res_neg, aes(y=term_name,x=log_p))+geom_bar(stat = 'identity')+xlab('-log(p value)')+
  theme_classic()+ylab('')+ggtitle(paste0(title, '(negative loading)'))
