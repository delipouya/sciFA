...
for i in range(num_sim_rounds):
    ...
    for i in range(num_factors):
        a_random_factor, overlap_matrix, mu_list, sigma_list, p_list = get_simulated_factor_object(n=num_samples, num_mixtures=num_mixtures, 
    ...
    corr_df_temp['SV_cov1'] = [np.corrcoef(overlap_mat_flat, SV_all_factors[0])[0,1]]
    corr_df_temp['SV_cov2'] = [np.corrcoef(overlap_mat_flat, SV_all_factors[1])[0,1]]
    ...
    corr_df_temp['1-AUC_cov1'] = [np.corrcoef(overlap_mat_flat, AUC_all_factors_df_1.iloc[0,:])[0,1]]
    corr_df_temp['1-AUC_cov2'] = [np.corrcoef(overlap_mat_flat, AUC_all_factors_df_1.iloc[1,:])[0,1]]
...
### save as a csv file
corr_df.to_csv('metric_overlap_corr_df_sim'+str(num_sim_rounds)+'.csv')